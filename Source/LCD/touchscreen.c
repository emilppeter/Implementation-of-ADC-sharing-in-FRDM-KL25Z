...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	if (b>0) {
		...
		dest_message.ChannelNum=LCD_TS_YU_CHANNEL;
		dest_message.TID=osThreadGetId();
		dest_message.ResultPtr=&x;
		dest_message.Flagnum=0x0001U;
		osMessageQueuePut(Message_id,&dest_message,1,osWaitForever);
		osThreadFlagsWait(dest_message.Flagnum,osFlagsWaitAll,osWaitForever);
		...
		dest_message.ChannelNum=LCD_TS_XL_CHANNEL;
		dest_message.TID=osThreadGetId();
		dest_message.ResultPtr=&y;
		dest_message.Flagnum=0x0001U;
		osMessageQueuePut(Message_id,&dest_message,1,osWaitForever);
		osThreadFlagsWait(dest_message.Flagnum,osFlagsWaitAll,osWaitForever);
